...
A real account-based music companion: Capture → choose → open provider → remember → rediscover. Six implemented spaces: Crate, Tonight, Atlas, Capsules, Listening and Karina. Personal collections and the listening archive are account-backed in D1, with private capsule images in R2. The labelled sample remains device-local. No Spotify credentials or AI calls are required. The personal and sample stores must never mix. Sample notes/activity are fictional and clearly labelled.
...
Spotify link handoff remains available; encrypted S256 OAuth supports current-playing display only and is enabled for the configured development app. The owner authorized through Resonance, then the website and Karina's `/fm` verified empty playback and the same real playing track/artist/Spotify link. User tokens and the verifier stay encrypted on the server; PKCE does not require copying the Spotify client secret. Spotify export analytics are explicitly blocked in the API pending verified permission. MetaBrainz live calls stay disabled in distributed hosting until the real contact/service terms/shared rate limiting are configured. ListenBrainz JSON imports work now and must retain their provenance and incomplete coverage. Never estimate minutes, count clicks as listening, or infer emotions/audio features.
...
The bounded subagent assessment is recorded in docs/ACCOUNT_ONBOARDING_NEXT.md. The current Discord connection is now verified; account-backed collections and additional sign-in methods are the next implementation phase.
...
Discord application Karina (1546493242865094766) was created with the user’s explicit approval. Its secret is stored in the hosting secret store, 13 commands are registered, Discord validated the public endpoint, and live identity linking, user installation and /stats, /chart and private /privacy replies were verified in Karina’s own DM. Spotify application Resonance (9f0238ce5bba4ed0bbfabcda68fbfb92) is now configured with the user's developer-agreement approval and a verified live account link. Last.fm configuration and unattended scheduling remain unfinished; no complete .fmbot premium parity claim. The user explicitly approved keeping the existing public Site audience and publishing. See docs/QA.md for precise tested limits.

New code: lib/karina (history, statistics, providers, AES-GCM, Discord protocol, PNG renderer, SQL guard helpers); app/api/karina; components/karina; db/schema.ts; immutable generated drizzle migrations; karina-worker signature-verifying relay and 5-minute cron; scripts/register-karina.ts dry-run default. No raw Spotify export uploads. Account-owned collection and journal sync was added on 8 September 2026. See docs/QA.md for measured checks.
...
## Simpler account onboarding, 8 September 2026

Implemented optional ChatGPT sign-in on the existing public Site, automatic per-user room creation, optional Spotify/Discord connection choices, returning-user room restoration, account settings, exports, sign-out and account-data deletion. Lifetime Spotify history uploads remain deferred by the user. Direct Google/Apple/Spotify login is unimplemented; current official Sites docs mention external providers but do not expose a confirmed integration procedure. Never label a music-provider link as an independent login.

`lib/account/collection.ts` stores revision-checked D1 document chunks and private R2 image documents. `rooms` revisions are unguessable and never reused after deletion. Migrations 0004 and 0005 add room metadata/chunks and a durable image cleanup queue. Existing archive owner IDs and encrypted connections are preserved. Background cleanup uses the existing secret-protected job endpoint; it does not change the live Cloudflare timer or expose a new setup URL.

`lib/resonance/use-collection.ts` now uses account persistence for personal mode and retains local storage only for the sample and explicit older-device recovery. Never silently attribute the origin-wide legacy collection. Keep async persistence acknowledgement before editor/WebMCP success. Account switching must hide portals as well as the room and invalidate pending work; same-account focus/visibility changes must retain drafts. History exports pin their initiating account across all pages.
