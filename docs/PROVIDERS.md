...
| Spotify API | Owner-scoped current-playing OAuth display | Configured; live owner authorization, empty and matching playing-track results in website/Discord verified | Additional users need development approval; live refresh and other clients remain unverified |
...
The [July 2026 changelog](https://developer.spotify.com/documentation/web-api/references/changes/july-2026) exists and describes development quota changes. Current [quota modes](https://developer.spotify.com/documentation/web-api/concepts/quota-modes) impose development owner/Premium/allowlist conditions and a separate extended-access process. Resonance's shared development application is configured. The owner completed website authorization and verified identity retrieval. Empty playback and matching real track/artist/Spotify links were verified in the website and Karina's own DM. Wider public Spotify access and additional real users are not verified.

The [developer policy](https://developer.spotify.com/policy) constrains derived analytics, AI ingestion, playback, monetization, attribution and deletion. Saved-link handoffs and explicitly labelled search links work without API access. An optional current-playing connection uses server-side S256 OAuth, verifies the immutable `account_id` from [the profile endpoint](https://developer.spotify.com/documentation/web-api/reference/get-current-users-profile), and stores encrypted tokens per Resonance owner. It requests only `user-read-currently-playing`; email, playback control and historical analytics are not requested. The website checks playback on opening the panel or manual refresh; Karina checks it on `/fm`. These snapshots are transient. Disconnect removes identity/tokens and suppresses in-flight display results. An API key is not user consent.
...
Implemented: Last.fm verified WebAuth, paginated existing-history backfill and continuing sync; Discord identify-only account linking and user-installed public command replies in DM/group/server contexts; Spotify S256 OAuth and encrypted refreshable current-playing display. Discord and Spotify connections are configured and tested live within the scope above; Last.fm and unattended scheduling remain pending operator setup. Spotify-derived analytics and Spotify archive uploads are blocked pending verified permission; original PNG/SVG charts use independently permitted imported/scrobbled records only. No provider album artwork is reused; official Spotify branding attributes its playback.
