async (page) => {
  ...
  await page
    ...
    .fill('https://www.youtube.com/watch?v=dQw4w9WgXc');
  await page
  ...
  await page.getByRole('textbox', { name: /Recommended by/ }).fill('A friend');
  await page
  ...
  check(
    (await page
      ...
      .getAttribute('href')) === 'https://www.youtube.com/watch?v=dQw4w9WgXc',
  ...
  await page.getByLabel('Moment type', { exact: true }).selectOption('session');
  ...
  console.log(
    'PASS: save → capsule → reload → provider handoff without listen → manual session → duplicate → undo → Tonight → Atlas → capsule reopen/duplicate/export → backup → lower effects → Unicode search → mobile',
  );
};
